...
inline std::array<int, N> calc_strides(const std::array<int, N> &dims)
{
    ...
    strides[0] = 1;
    for (int i = 1; i < N; ++i) strides[i] = strides[i - 1] * dims[i - 1];
    ...
    const std::array<int, N> &offsets)
{
    ...
    for (int i = 0; i < N; ++i) indices[i] += offsets[i] + 1;
...
{
    int product = array[0];
    for (int i = 1; i < N; ++i) product *= array[i];
...
public:
    ...
    Array(const std::vector<T> &data, const std::array<int, N> &dims)
            : dims(dims),
              ...
              data(
                  data.begin(),
                  data.begin() + ncells), // Do not copy beyond the end.
              strides(calc_strides<N>(dims)),
              offsets({})
    ...
    Array<T, N> &operator=(const Array<T, N> &) =
        default; // CvH does this one need empty checking?
    ...
    inline T max() const { return *std::max_element(data.begin(), data.end()); }

    inline T min() const { return *std::min_element(data.begin(), data.end()); }
    ...
    inline Array<T, N>
    subset(const std::array<std::pair<int, int>, N> ranges) const
    {
        ...
        for (int i = 0; i < a_sub.ncells; ++i) {
            ...
            for (int n = N - 1; n > 0; --n) {
                indices[n] =
                    do_spread[n] ? 1 : ic / a_sub.strides[n] + ranges[n].first;
                ic %= a_sub.strides[n];
            }
