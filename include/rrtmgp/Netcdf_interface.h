/*
 * This file is imported from MicroHH
 * (https://github.com/earth-system-radiation/earth-system-radiation) and is
 * adapted for the testing of the C++ interface to the RTE+RRTMGP radiation
 * code.
 *
...
public:
    ...
    get_variable_dimensions(const std::string &) const;

    bool variable_exists(const std::string &) const;

    template<typename T>
    Netcdf_variable<T>
    add_variable(const std::string &, const std::vector<std::string> &);

    template<typename T>
    T get_variable(const std::string &) const;

    template<typename T>
    std::vector<T>
    get_variable(const std::string &, const std::vector<int> &) const;
    ...
    void add_attribute(const std::string &, const std::string &, const int);

    void add_attribute(const std::string &, const double, const int);

    void add_attribute(const std::string &, const float, const int);
...
{
    if (return_value != NC_NOERR) nc_throw(return_value);
}

// Get the NetCDF data type based on TF
template<typename TF>
nc_type netcdf_dtype();
template<>
nc_type netcdf_dtype<double>()
{
    return NC_DOUBLE;
}
template<>
nc_type netcdf_dtype<float>()
{
    return NC_FLOAT;
}
template<>
nc_type netcdf_dtype<int>()
{
    return NC_INT;
}
        ...
        : Netcdf_handle()
{
    ...
    if (mode == Netcdf_mode::Create)
        nc_check_code =
            nc_create(name.c_str(), NC_NOCLOBBER | NC_NETCDF4, &ncid);
    else if (mode == Netcdf_mode::Write)
        nc_check_code = nc_open(name.c_str(), NC_WRITE | NC_NETCDF4, &ncid);
    else if (mode == Netcdf_mode::Read) {
        // CvH: I changed this line, as I can no longer open the "classic"
        // coefficient files. nc_check_code = nc_open(name.c_str(), NC_NOWRITE |
        // NC_NETCDF4, &ncid);
    ...
    try {
        ...
        std::string error =
            "Opening of file " + name + " returned: " + e.what();
        throw std::runtime_error(error);
    }

    root_ncid = ncid;

    if (mode == Netcdf_mode::Create) nc_check_code = nc_enddef(root_ncid);
...
inline void
Netcdf_handle::add_dimension(const std::string &dim_name, const int dim_size)
{
    int nc_check_code = 0;
    ...
    const std::vector<std::string> &dim_names)
{
    ...
    // Broadcast the dim_ids size of the main process to run the for loop on all
    // processes.
        ...
        if (dim_len_int == NC_UNLIMITED) {
            if (i == 0)
                ...
                throw std::runtime_error(
                    "Only the leftmost dimension is allowed to be an "
...
inline Netcdf_handle::Netcdf_handle() : record_counter(0) {}
    ...
    const std::vector<int> &i_count)
{
    ...
    nc_check_code = nc_put_vara_wrapper<T>(
        ncid,
        var_id,
        i_start_size_t,
        i_count_size_t,
    ...
    const int var_id)
{
    ...
    // CvH what if string is too long?
    nc_check_code =
        nc_put_att_double(ncid, var_id, name.c_str(), NC_DOUBLE, 1, &value);
    nc_check(nc_check_code);
    ...
    const int var_id)
{
    ...
    // CvH what if string is too long?
    nc_check_code =
        nc_put_att_float(ncid, var_id, name.c_str(), NC_FLOAT, 1, &value);
    nc_check(nc_check_code);
...
Netcdf_handle::get_variable_dimensions(const std::string &name) const
{
    int nc_check_code = 0;
    int var_id;
    ...
    for (int n = 0; n < ndims; ++n) {
        ...
        nc_check_code =
            nc_inq_dim(ncid, dimids[n], dim_name, &dim_length_size_t);
        nc_check(nc_check_code);
...
template<typename TF>
inline TF Netcdf_handle::get_variable(const std::string &name) const
{
    ...
    const std::vector<int> &i_count) const
{
    ...
    // CvH check needs to be added if total count matches multiplication of all
    // dimensions.

    std::vector<TF> values(total_count);
    nc_check_code = nc_get_vara_wrapper(
        ncid,
        var_id,
        i_start_size_t,
        i_count_size_t,
    ...
    const std::vector<int> &i_count) const
{
    ...
    try {
        ...
        std::string warning =
            "Netcdf variable: " + name + " not found, filling with zeros";
...
template<typename T>
inline Netcdf_variable<T>::Netcdf_variable(
    Netcdf_handle &nc_file,
    const int var_id,
    const std::vector<int> &dim_sizes)
        : nc_file(nc_file),
...
template<typename T>
inline void
Netcdf_variable<T>::insert(const T value, const std::vector<int> i_start)
{
    nc_file.insert(value, var_id, i_start, dim_sizes);
}

template<typename T>
inline void Netcdf_variable<T>::add_attribute(
    const std::string &name,
    const std::string &value)
{
...
template<typename T>
inline void
Netcdf_variable<T>::add_attribute(const std::string &name, const double value)
{
    nc_file.add_attribute(name, value, var_id);
}
...
template<typename T>
inline void
Netcdf_variable<T>::add_attribute(const std::string &name, const float value)
{
    nc_file.add_attribute(name, value, var_id);
}
#endif
